 ...
 *************************************************/
const SceneManager = (canvas, addNodeRequest, selectNode, selectElement, addLogEntry, moveNode, modelProps) => {
    ...
    function onMouseDown(x: number, y: number) {
        ...
        if(modelProps.fe3Mesh.stage > 1) {
            ...
            for ( let i = 0; i < intersects.length; i++ ) {
                ...
                if(clickedObj.userData.objType == 'node' && clickedObj.userData.isMoveable){
    ...
    function onMouseUp(x: number, y: number) {
        ...
        // for ( let i = intersects.length-1; i >= 0; i-- ) {
            ...
            const res  = {
                ...
                distanceCentre: ThreeSF(clickedObj.position.distanceTo(intersects[ i ].point))
            }
            console.log(res)  
            ...
            if(clickedObj.userData.objType == 'linkbar'){
                if(modelProps.fe3Mesh.stage === 1) {
                    ...
                    let boundaryAxis = getAxis(clickedObj.userData.isBoundaryV, clickedObj.userData.isBoundaryH)
                    ...
                    if(boundaryAxis) {
                        ...
                        let scaledPos = (ud.height * (1.00 - intersects[ i ].uv.y)) / ud.xyScale;
            ...
            if(clickedObj.userData.objType == 'backplane' && !clickedObj.userData.isPerimeter){
            ...
            if(clickedObj.userData.objType == 'element' && mouseStatus.nodeToMove < 1){
                //clickedObj.material.color.set( THREE_COLOURS.NODE_SELECTED ) //Set the selected node colour, better to let state do this
    ...
    function FindRaycasterIntersections(x: number, y:number){
        ...
        ndcMouse.x = ( x1 / canvas.width ) * 2 - 1;
        ndcMouse.y = - ( y1 / canvas.height ) * 2 + 1;
    ...
    //Function not needed!!!!!!!!!!!!!!
    function findClickedCylinderYPosition(clickedCylinder: THREE.Object3D, clickPoint){
        let pivotGroup = new THREE.Group()
        pivotGroup.position.set(clickedCylinder.position.x, clickedCylinder.position.y, clickedCylinder.position.z) 
        ...
        console.log("clicked Cyl position:" + yDistance + " height:" + yCylHeight + " pos: " + pos)
        return pos
    }
