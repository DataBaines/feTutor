...
export class FeElement {
	...
	calcArea()
	{
        ...
        let term1: number; let term2: number; let term3: number;
	    let a: number; let b: number; let c: number;
        let d: number; let e: number; let f: number;
        let g: number; let h: number; let i: number;
...
//		System.out.println("Area= " + area);
        ...
        let k: number; let l: number; let m: number;
        let n: number; let o: number; let p: number;
        let q: number; let r: number; let s: number;

		k = e*i-h*f;  l = (d*i-g*f)*-1;  m = d*h-g*e;
		n = (b*i-h*c)*-1;	o = a*i-g*c;  p = (a*h-g*b)*-1;
		q = b*f-e*c;  r = (a*f-d*c)*-1;  s = a*e-d*b;

		let m1: number[][] = [[k, l, m],[n, o, p],[q, r, s]];
		// m1[0][0] = k;
		// m1[0][1] = l;
		// m1[0][2] = m;
		// m1[1][0] = n;
