...
export class FeMesh {
  ...
  generateMesh() {
    ...
    if (this.verNodes < this.horNodes) this.numUp = true;
    else this.numUp = false;
  ...
  public createPerimeterNodes(axis: string, pos: number) {
    if (axis == "x" && this.horNodes < FeMesh.MAXNODES) {
      ...
      this.countX++;
      this.nodeXPerimeter[this.countX] = new FeNode(
        pos,
        this.height,
        false,
        true
    ...
    if (axis == "y" && this.verNodes < FeMesh.MAXNODES) {
      ...
      this.countY++;
      this.nodeYPerimeter[this.countY] = new FeNode(
        this.width,
        pos,
        true,
        false
  ...
  private sortXPerimeterNodes() {
    ...
    for (
      let i: number = 1;
      i < this.horNodes - 1;
      i++ // first and last are corners, so ignore
    ) {
      lowVal = this.width;
      found = false;
      for (
        let j: number = 0;
        j < this.countX;
        j = j + 2 //Created in pairs
      ) {
        if (
          this.nodeXPerimeter[j].getX() < lowVal &&
          this.nodeXPerimeter[j].getX() > prev
        ) {
  ...
  private sortYPerimeterNodes() {
    ...
    for (let i: number = 1; i < this.verNodes - 1; i++) {
      ...
      for (let j: number = 0; j < this.countY; j = j + 2) {
        if (
          this.nodeYPerimeter[j].getY() < lowVal &&
          this.nodeYPerimeter[j].getY() > prev
        ) {
  ...
  private createElements() {
    ...
    for (let i: number = 0; i < this.verNodes - 1; i++) {
      for (let j: number = 0; j < this.horNodes - 1; j++) {
        ...
        this.element[elNo] = new FeElement(
          elNo,
          this.nodes2D[i][j],
          this.nodes2D[i][j + 1],
          this.nodes2D[i + 1][j + 1]
        );
        this.element[elNo + 1] = new FeElement(
          elNo + 1,
          this.nodes2D[i + 1][j + 1],
          this.nodes2D[i + 1][j],
          this.nodes2D[i][j]
  ...
  public moveInnerNode(nodeID: number, x: number, y: number) {
    ...
    console.log(
      `femesh.moveInnerNode old x: ${this.nodes[
        nodeID
      ].getX()},  y: ${this.nodes[nodeID].getY()}`
    );
    ...
    for (i = 0; i < this.horNodes; i++) {
      for (j = 0; j < this.verNodes; j++) {
        ...
        if (i < this.horNodes - 1) {
          ...
          json = this.buildThreeLinkbarObject(
            node1,
            node2,
            false,
            this.isHorBoundary(j)
          );
          allLinkbars.push(json);
        }
  ...
  perimeterValuesEntered() {
    ...
    for (i = 0; i < this.verNodes; i++) {
      if (!this.nodes2D[i][0].hasVal()) ok = false;
      if (!this.nodes2D[i][this.horNodes].hasVal()) ok = false;
    }
    for (j = 0; j < this.horNodes; j++) {
      if (!this.nodes2D[0][j].hasVal()) ok = false;
      if (!this.nodes2D[this.verNodes][j].hasVal()) ok = false;
    }
    return ok;
  }
  ...
  onVerticalPerimeter(num: number) {
    ...
    if (this.numUp) {
      ...
      if (num % this.horNodes == 0 || num % this.horNodes == 1) flag = true;
    }
    return flag;
  }
  ...
  onHorizontalPerimeter(num: number) {
    ...
    if (this.numUp) {
      if (num % this.verNodes == 0 || num % this.verNodes == 1) flag = true;
    } else {
  ...
  private evalBoundary() {
    ...
    for (let i: number = 0; i < this.verNodes; i++) {
      // left side
      y = this.nodes2D[i][0].getY();
      res = this.evalPoint(y, this.lefc, this.lef1, this.lef2);
      this.nodes2D[i][0].setValue(res);

      // right side
      y = this.nodes2D[i][this.horNodes - 1].getY();
      res = this.evalPoint(y, this.rigc, this.rig1, this.rig2);
      this.nodes2D[i][this.horNodes - 1].setValue(res);
    }
    for (let j: number = 1; j < this.horNodes - 1; j++) {
      // bottom side
      x = this.nodes2D[0][j].getX();
      res = this.evalPoint(x, this.botc, this.bot1, this.bot2);
      this.nodes2D[0][j].setValue(res);

      // top side
      x = this.nodes2D[this.verNodes - 1][j].getX();
      res = this.evalPoint(x, this.topc, this.top1, this.top2);
  ...
  gaussSeidel() {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (Math.abs(this.matrix[i][i]) < s * 0.75) k = 0;
      s = 0;
    }
    //Will not converge
    if (k != 12) {
    ...
    for (i = 0; i < n; i++) this.nodes[i + 1].setValue(x[i]);
